...
def try_gpu_mps(i=0):  # mac 无法使用 cuda 加速，只有 mps 加速；对应 d2l 原函数 try_gpu()
    """如果存在, 则返回gpu(i), 否则返回cpu()"""
    if torch.mps.device_count() >= i + 1:
        return torch.device(f'mps:{i}')
    return torch.device('cpu')

def try_all_gpus_mps():  # mac 无法使用 cuda 加速，只有 mps 加速；对应 d2l 原函数 try_all_gpus()
    """返回所有可用的GPU, 如果没有GPU, 则返回[cpu(),]"""
    devices = [torch.device(f'mps:{i}') for i in range(torch.mps.device_count())]
    return devices if devices else [torch.device('cpu')]

# 解决 wikitext-2-v1 无法使用的问题: https://discuss.d2l.ai/t/wikitext-2-v1/22813/1
